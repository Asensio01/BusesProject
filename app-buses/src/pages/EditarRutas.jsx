  ...
  const handleEditarRuta = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/rutas/${rutaSeleccionada}`,
        {
          method: "PUT",
      ...
      if (!response.ok) {
        throw new Error(
          `Error ${response.status}: No se pudo actualizar la ruta.`
        );
      }
      ...
      setRutas((prevRutas) =>
        prevRutas.map((ruta) =>
          ruta.idRuta.toString() === rutaSeleccionada
            ? { ...ruta, nombreRuta: nuevoNombreRuta }
            : ruta
        )
      );
    ...
    <div className="crear-rutas-container">
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <button
          ...
          disabled={cargando}
          onClick={() => setConfirmarEdicion(true)}
        >
      ...
      {confirmarEdicion && (
        <div className="popup">
          <div className="popup-content">
            ...
            <p>
              Nombre nuevo: <strong>{nuevoNombreRuta}</strong>
            </p>
            <button className="btn-primary" onClick={handleEditarRuta}>
              Confirmar
            </button>
            <button
              className="btn-cancel"
              onClick={() => setConfirmarEdicion(false)}
            >
              Cancelar
            </button>
          </div>
