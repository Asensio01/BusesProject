...
function AsignarTramos() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (asignacionDuplicada()) {
      setMensajeError("⚠️ No se puede asignar el mismo tramo a la misma ruta con la misma hora.");
    ...
    try {
      ...
      const rutaTramoResponse = await fetch("http://localhost:8080/api/rutas-tramos/con-tipo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!rutaTramoResponse.ok) throw new Error("No se pudo asignar el tramo a la ruta.");
