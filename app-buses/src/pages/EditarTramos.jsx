...
function EditarTramos() {
  ...
  useEffect(() => {
    if (asignacionSeleccionada) {
      const asignacion = asignaciones.find(
        (a) => a.idRutaTramo.toString() === asignacionSeleccionada
      );
      if (asignacion) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (asignacionDuplicada()) {
      setMensajeError(
        "⚠️ No se puede actualizar, ya existe una asignación con estos datos."
      );
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/rutas-tramos/${asignacionSeleccionada}`,
        {
          method: "PUT",
        ...
        >
          ...
          {asignaciones.map((asignacion) => (
            <option key={asignacion.idRutaTramo} value={asignacion.idRutaTramo}>
              {asignacion.ruta.nombreRuta} -{" "}
              {asignacion.tramo.ciudadOrigen.nombre} →{" "}
              {asignacion.tramo.ciudadDestino.nombre} (
              {asignacion.tramo.horaSalida})
            </option>
          ))}
        </select>
